...
float2 randomFloat2(float2 uv) {
	return float2(randomFloat(uv), randomFloat(uv.yx));
}
...
void RayGen()
{
	...
	for (int r = 0; r < raysPerPixel; r++) {
		...
		float ray01 = r / raysPerPixel;
		adjustedIndices += randomFloat2(rayIndices.xy * ray01);
...
void ClosestHit(inout RayPayload payload, BuiltInTriangleIntersectionAttributes hitAttributes)
{
	...
	float2 rng = randomFloat2(pixelUV * (payload.recursionDepth + 1) + payload.rayPerPixelIndex + RayTCurrent());
